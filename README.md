...
- **auto_fade -** Boolean, defaults to False. If True, then an alternative mode is used: unmentioned_indices will be introduced/removed by the default introducer/remover, and there is no need or check for the unmentioned indices to be the same length. When both this and auto_morph are False, then the default behavior applies: the unmentioned indices will be ReplacementTransformed into one another if they are the same length, or will trigger show_indices mode if they are not the same length.
- **auto_morph -** Boolean, defaults to False. If True, then an alternative mode is used: unmentioned_indices will be transformed into one another as one large group, and there is no need or check for the unmentioned indices to be the same length. When both this and auto_fade are False, then the default behavior applies.
- **auto_resolve_kwargs -** Dictionary, defaults to {}. These kwargs are applied to the animations resolving the unmentioned indices. If auto_fade is True, these are the FadeIn and FadeOut of each unmentioned glyph. If auto_morph is True, this is the ReplacementTransform mapping the entire group of unmentioned indices to one another. If both are False, then this is the ReplacementTransforms mapping individual glyphs to one another.
- **show_indices -** Boolean, defaults to False. If True, then the results of the glyph_map are ultimately discarded (although it is still processed) and the indices of the submobjects being operated on are shown. This is useful for writing the glyph_map in the first place, making it easy to see which indices need to go where and in what way. This mode can also be triggered by an empty glyph_map, by the presence of an empty glyph_map entry `([], [])`, or by a mismatch in the number of indices not mentioned in the glyph_map (if auto_fade and auto_morph are False).
